...
function start_timer() {
  play_button.disabled="true" ;//Le bouton play est désactivé lorsqu'on lance la fonction associée au clic
  pause_button.disabled="false" ;//Le bouton pause est rendu actif
...
function pause_timer() {
  pause_button.disabled="true" ;// De même, le bouton pause est désactivé au moment où le timer est mis en pause
  play_button.disabled = "false" ;// Le bouton play est réactivé
  t.stop();
...
function increment() {
  if (year_current === year_max) {
    t.stop();
    play_button.disabled = "false" ;// Le bouton est réactivé une fois le traitement terminé
    pause_button.disabled = "true" ;//Idem, le bouton pause est désactivé, retour à la situation initiale
  } else {
    year_current += 1;
    year_index = year_current - year_min;
